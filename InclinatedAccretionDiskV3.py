...
Para [0,pi/2] usamos as transfer function TF1, TF2, TF3
Para [3/2pi, 2pi] usamos as transfer functions TF4, TF5, TF6
...
for b in ListaParametros:
    ...
    for theta in Angulos:
        ...
        #Agora terceiro e quarta quadrantes
        ...
        if len(sol.t_events[1])!=0: #TF1 ou TF4
            ...
            if EH==0 or sol.t_events[1][0] < ParametroEH:
                #Terceiro quadrante
                ObsInt[j][k] += (sqrt(1-(2*M)/r)**4)*SourceProfile(r)
                #Quarto quadrante
                if theta!=pi/2:
                    pos = j % (2*NumAngulos-2)
